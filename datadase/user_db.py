...
def select_name_address_title_from_users(user_id):
    cur.execute("""SELECT user_full_name, user_address, user_title FROM users WHERE user_id = %s""", (user_id,))
    return cur.fetchall()
...
def register_or_update_user_data(user_id, value, name=False, address=False, register=False, title=False):
    if register:
        ...
        if title:
            cur.execute("""INSERT INTO users (user_id, user_title) VALUES (%s, %s)""", (user_id, value))
        ...
        if title:
            cur.execute("""UPDATE users SET user_title = %s WHERE user_id = %s""", (value, user_id))
