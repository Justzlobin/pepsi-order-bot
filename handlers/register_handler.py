...
def register_register_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(user_register_name, callback=cat_cb.filter(action='register_user_name'))
    dp.register_message_handler(name_chosen, UserRegisterName.user_choosing_name)
